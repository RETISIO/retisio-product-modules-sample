package com.retisio.arc.projection.handler;
...
public class CatalogProjectionHandler extends R2dbcHandler<EventEnvelope<CatalogEvent>> {
    ...
    private CompletionStage<Done> processReadSide(R2dbcSession session, CatalogEvent event){
        if(event instanceof CatalogEvent.CatalogCreated) {
            ...
            return processReadSideExt(session, event);
        }
    }
    ...
    private static final String SAVE_CATALOG_QUERY = "INSERT INTO CATALOG(" +
            ...
            "DYNA_PROPS=excluded.DYNA_PROPS, " +
            "IS_DELETED=excluded.IS_DELETED, " +
            "LAST_MODIFIED_TMST=now()";
    protected CompletionStage<Done> insertCatalog(R2dbcSession session, CatalogEvent.CatalogCreated event) {
        ...
        statementWrapper.bind(index.incrementAndGet(), toJsonString(event.getDynProps()), String.class);
    ...
    protected CompletionStage<Done> updateCatalog(R2dbcSession session, CatalogEvent.CatalogUpdated event) {
    ...
    protected CompletionStage<Done> patchCatalog(R2dbcSession session, CatalogEvent.CatalogPatched event) {
        ...
        if(Objects.nonNull(event.getDynProps())) {
            query.append("DYNA_PROPS = $"+q_index.incrementAndGet()+", ");
        }
    ...
    protected CompletionStage<Done> deleteCatalog(R2dbcSession session, CatalogEvent.CatalogDeleted event) {
    ...
    private CatalogMessage convertToCatalogPublishEvent(CatalogEvent event, Catalog catalog) {
        if(event instanceof CatalogEvent.CatalogCreated){
            ...
            return convertToCatalogPublishEventExt(event, catalog);
        }
    }
    ...
    protected CatalogMessage.Catalog convertToCatalogMessage(Catalog catalog) {
