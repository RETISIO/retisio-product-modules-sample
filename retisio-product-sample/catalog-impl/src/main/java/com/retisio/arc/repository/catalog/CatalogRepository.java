...
public class CatalogRepository {
    ...
    private String getCatalogsQuery(Optional<String> filter, String pageSize, String offSetValue) {
        String filterQuery = filter
                ...
                .filter(s -> Objects.nonNull(s) && !s.trim().equals(""))
                .map(c -> "where "+c)
                .orElseGet(()->"");

        return "select catalog_id, catalog_name, is_active,DYNA_PROPS, is_deleted, COUNT(1) OVER () as TOTAL_COUNT from catalog " +
                ...
                r2dbcSession.select(
                        ...
                        row -> {
                            ...
                            return GetCatalogResponse.builder()
                                    ...
                                    .dynProps(toObject(row.get("DYNA_PROPS", String.class), LinkedHashMap.class))
